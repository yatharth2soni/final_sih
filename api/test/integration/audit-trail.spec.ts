...
describe('Hash-Chained Tamper-Evident Audit Trail (Stretch Phase)', () => {
  ...
  beforeAll(async () => {
    ...
    const corpA = await prisma.user.upsert({
      ...
      create: { name: 'Corp A Audit', email: 'corp-a-aud@coalindia.gov.in', passwordHash, role: 'CORPORATE_MANAGEMENT', companyId: companyAId, status: 'ACTIVE' },
    });
    ...
    await prisma.user.upsert({
      ...
      create: { name: 'Corp B Audit', email: 'corp-b-aud@coalindia.gov.in', passwordHash, role: 'CORPORATE_MANAGEMENT', companyId: companyBId, status: 'ACTIVE' },
    });
