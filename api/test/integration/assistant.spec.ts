...
describe('Multilingual Governed Conversational Data Interface (Final Stretch Phase)', () => {
  ...
  beforeAll(async () => {
    ...
    const corpA = await prisma.user.upsert({
      ...
      create: { name: 'Corp A Asst', email: 'corp-a-asst@coalindia.gov.in', passwordHash, role: 'CORPORATE_MANAGEMENT', companyId: companyAId, status: 'ACTIVE' },
    });

    const corpB = await prisma.user.upsert({
      where: { email: 'corp-b-asst@coalindia.gov.in' },
      update: { passwordHash, companyId: companyBId },
      create: { name: 'Corp B Asst', email: 'corp-b-asst@coalindia.gov.in', passwordHash, role: 'CORPORATE_MANAGEMENT', companyId: companyBId, status: 'ACTIVE' },
    });
  ...
  describe('1. Bilingual Conversational Queries (English & Hindi)', () => {
    ...
    it('Accepts short greetings like "hi" and returns HELP_CAPABILITIES (200 OK)', async () => {
      const res = await request(app.getHttpServer())
        .post('/api/v1/assistant/query')
        .set('Authorization', `Bearer ${officialToken}`)
        .send({ question: 'hi' })
    ...
    it('Dynamically extracts mine entity from natural language query', async () => {
      const res = await request(app.getHttpServer())
        .post('/api/v1/assistant/query')
        .set('Authorization', `Bearer ${officialToken}`)
        .send({
          question: 'What is the risk score for Assistant Mine Alpha?',
        })
        .expect(200);

      expect(res.body.data.intent).toBe('MINE_RISK');
      expect(res.body.data.answer).toContain('Assistant Mine Alpha');
      expect(res.body.data.answer).toContain('82 / 100');
    });

    it('Handles multi-mine risk queries (e.g. "Which mines are high risk?")', async () => {
      const res = await request(app.getHttpServer())
        .post('/api/v1/assistant/query')
        .set('Authorization', `Bearer ${officialToken}`)
        .send({
          question: 'Which mines are high risk in our scope?',
        })
        .expect(200);

      expect(res.body.data.intent).toBe('MINE_RISK');
      expect(res.body.data.answer).toContain('Mine Safety Risk Summary');
      expect(res.body.data.answer).toContain('Assistant Mine Alpha');
    });
  });
  ...
  describe('2. Scope Enforcement & Prompt Injection Defense', () => {
    ...
    it('Malicious prompt injection attempts are safely classified as unknown/help without leaking secrets', async () => {
      ...
      expect(res.body.data.answer).toContain('This AI assistant is designed specifically for Khanan Suraksha');
    });

    it('Rejects questions shorter than 2 characters (400 Bad Request)', async () => {
      await request(app.getHttpServer())
        .post('/api/v1/assistant/query')
        .set('Authorization', `Bearer ${officialToken}`)
        .send({ question: 'a' })
