...
describe('Worker Attendance (Phase 5)', () => {
  ...
  beforeAll(async () => {
    ...
    const corpA = await prisma.user.upsert({
      ...
      create: { name: 'Corp A Att', email: 'corp-a-att@coalindia.gov.in', passwordHash, role: 'CORPORATE_MANAGEMENT', companyId: companyAId, status: 'ACTIVE' },
    });

    const corpB = await prisma.user.upsert({
      where: { email: 'corp-b-att@coalindia.gov.in' },
      update: { passwordHash, companyId: companyBId },
      create: { name: 'Corp B Att', email: 'corp-b-att@coalindia.gov.in', passwordHash, role: 'CORPORATE_MANAGEMENT', companyId: companyBId, status: 'ACTIVE' },
    });
