...
describe('Contractor Management (Phase 4)', () => {
  ...
  beforeAll(async () => {
    ...
    const corpA = await prisma.user.upsert({
      ...
      create: { name: 'Corp A Contr', email: 'corp-a-contr@coalindia.gov.in', passwordHash, role: 'CORPORATE_MANAGEMENT', companyId: companyAId, status: 'ACTIVE' },
    });

    const corpB = await prisma.user.upsert({
      where: { email: 'corp-b-contr@coalindia.gov.in' },
      update: { passwordHash, companyId: companyBId },
      create: { name: 'Corp B Contr', email: 'corp-b-contr@coalindia.gov.in', passwordHash, role: 'CORPORATE_MANAGEMENT', companyId: companyBId, status: 'ACTIVE' },
    });
