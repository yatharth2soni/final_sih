...
describe('Dashboard Aggregation and Statutory Reporting (Phase 8)', () => {
  ...
  beforeAll(async () => {
    ...
    const corpA = await prisma.user.upsert({
      ...
      create: { name: 'Corp A User', email: 'corp-a@coalindia.gov.in', passwordHash, role: 'CORPORATE_MANAGEMENT', companyId: companyAId, status: 'ACTIVE' },
    });
    ...
    const corpB = await prisma.user.upsert({
      ...
      create: { name: 'Corp B User', email: 'corp-b@coalindia.gov.in', passwordHash, role: 'CORPORATE_MANAGEMENT', companyId: companyBId, status: 'ACTIVE' },
    });
  ...
  describe('6. Statutory Risk Dossier PDF Generation (GET /api/v1/reports/risk-dossier/:mineId)', () => {
    it('Generate and download valid Risk Dossier PDF with binary application/pdf response', async () => {
      const res = await request(app.getHttpServer())
        .get(`/api/v1/reports/risk-dossier/${mineA1Id}`)
        .set('Authorization', `Bearer ${adminToken}`)
        .expect(200);

      expect(res.headers['content-type']).toContain('application/pdf');
      expect(res.headers['content-disposition']).toContain('attachment; filename="risk-dossier-');
      expect(res.headers['content-disposition']).toContain('.pdf"');

      // Binary PDF verification: valid PDF buffer starts with '%PDF-'
      const buffer = res.body instanceof Buffer ? res.body : Buffer.from(res.text || '', 'binary');
      expect(buffer.length).toBeGreaterThan(100);
      expect(buffer.toString('utf-8', 0, 5)).toBe('%PDF-');
    });

    it('Corporate User A can download their own mine dossier (200 OK)', async () => {
      const res = await request(app.getHttpServer())
        .get(`/api/v1/reports/risk-dossier/${mineA1Id}`)
        .set('Authorization', `Bearer ${corporateToken}`)
        .expect(200);
    ...
    it('Corporate User B is forbidden from downloading Mine A1 dossier (403 Forbidden)', async () => {
      await request(app.getHttpServer())
        .get(`/api/v1/reports/risk-dossier/${mineA1Id}`)
        .set('Authorization', `Bearer ${otherCorporateToken}`)
        .expect(403);
    ...
    it('Reject non-existent mine ID (404 Not Found)', async () => {
      await request(app.getHttpServer())
        .get('/api/v1/reports/risk-dossier/00000000-0000-0000-0000-000000000000')
        .set('Authorization', `Bearer ${adminToken}`)
        .expect(404);
    });
  });
