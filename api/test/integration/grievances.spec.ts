...
describe('Grievance Handling (Phase 6)', () => {
  ...
  beforeAll(async () => {
    ...
    const corpA = await prisma.user.upsert({
      ...
      create: { name: 'Corp A Grv', email: 'corp-a-grv@coalindia.gov.in', passwordHash, role: 'CORPORATE_MANAGEMENT', companyId: companyAId, status: 'ACTIVE' },
    });
    ...
    const corpB = await prisma.user.upsert({
      ...
      create: { name: 'Corp B Grv', email: 'corp-b-grv@coalindia.gov.in', passwordHash, role: 'CORPORATE_MANAGEMENT', companyId: companyBId, status: 'ACTIVE' },
    });
