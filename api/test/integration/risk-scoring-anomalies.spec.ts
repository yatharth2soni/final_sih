...
describe('Explainable Risk Scoring and Anomaly Detection (Phase 9)', () => {
  ...
  beforeAll(async () => {
    ...
    const corpA = await prisma.user.upsert({
      ...
      create: { name: 'Corp A Risk', email: 'corp-a-risk@coalindia.gov.in', passwordHash, role: 'CORPORATE_MANAGEMENT', companyId: companyAId, status: 'ACTIVE' },
    });
    ...
    const corpB = await prisma.user.upsert({
      ...
      create: { name: 'Corp B Risk', email: 'corp-b-risk@coalindia.gov.in', passwordHash, role: 'CORPORATE_MANAGEMENT', companyId: companyBId, status: 'ACTIVE' },
    });
