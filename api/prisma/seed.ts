...
async function main() {
  ...
  const secl = await prisma.company.upsert({
    where: { code: 'SECL' },
    update: {},
    create: {
  ...
  const ncl = await prisma.company.upsert({
    where: { code: 'NCL' },
    update: {},
    create: {
  ...
  const mcl = await prisma.company.upsert({
    where: { code: 'MCL' },
    update: {},
    create: {
  ...
  const ccl = await prisma.company.upsert({
    where: { code: 'CCL' },
    update: {},
    create: {
  ...
  const wcl = await prisma.company.upsert({
    where: { code: 'WCL' },
    update: {},
    create: {
  ...
  const sccl = await prisma.company.upsert({
    where: { code: 'SCCL' },
    update: {},
    create: {
  ...
  console.log(`  ✓ All CIL Subsidiaries & SCCL Registered in Database`);
  ...
  const users = [
    ...
    },
    {
      ...
      email: 'contractor@easterncoking.com',
      phone: '+91-9876543214',
      role: UserRole.CONTRACTOR,
      companyId: bccl.id,
      contractorId: 'CNT-2026-BCCL-089',
    },
  ];
  ...
  for (const u of users) {
    const user = await prisma.user.upsert({
      where: { email: u.email },
      update: { companyId: u.companyId, role: u.role, name: u.name, contractorId: u.contractorId },
      create: {
        name: u.name,
        email: u.email,
        phone: u.phone,
  ...
  const mineData = [
    ...
    },
    {
      ...
      code: 'BCCL-MN-UG01',
      location: 'Western Jharia, Dhanbad, Jharkhand',
      companyId: bccl.id,
      geoBoundary: { lat: 23.7389, lng: 86.3452, radius_km: 3.0 },
    },
    {
      name: 'Gevra Mega Expansion OCP',
      code: 'SECL-GEV-OCP',
      location: 'Korba Coalfield, Gevra, Chhattisgarh',
      companyId: secl.id,
      geoBoundary: {
        type: 'Polygon',
        coordinates: [
    ...
    {
      name: 'Kusmunda Open Cast Project',
      code: 'SECL-KUS-OCP',
      location: 'Kusmunda Area, Korba, Chhattisgarh',
      companyId: secl.id,
      geoBoundary: { lat: 22.3582, lng: 82.6841, radius_km: 4.0 },
    },
    {
      ...
      companyId: secl.id,
      geoBoundary: { lat: 22.3595, lng: 82.7501, radius_km: 2.5 },
    },
    {
    ...
    },
    {
      ...
      code: 'ECL-RAJ-OCP',
      location: 'Rajmahal Coalfield, Godda, Jharkhand',
      companyId: ecl.id,
      geoBoundary: { lat: 25.0418, lng: 87.3541, radius_km: 3.5 },
    },
    {
      name: 'Jayant Mega Open Cast Project',
      code: 'NCL-JAY-OCP',
      location: 'Singrauli Coalfield, Jayant, MP',
      companyId: ncl.id,
      geoBoundary: { lat: 24.1284, lng: 82.6418, radius_km: 4.0 },
    },
    {
  ...
  const jhariaId = mines['BCCL-JHA-BLK4'] || Object.values(mines)[0];
  const korbaId = mines['SECL-KRB-WP2'] || mines['BCCL-MN-UG01'] || Object.values(mines)[1];
  const mahapatraId = dbUsers['r.mahapatra@coalindia.gov.in'];
  const adminId = dbUsers['admin@coalmine.gov.in'];

  for (const mId of [jhariaId, korbaId].filter(Boolean)) {
  ...
  if (bcclCompany) {
    ...
    const contractor1 = await prisma.contractor.upsert({
      ...
      create: {
        companyId: bcclCompany.id,
        legalName: 'Demo Mining & Infrastructure Solutions (BCCL)',
        tradeName: 'Demo Mining Solutions',
        registrationNumber: 'CIN-U10100MH2018PTC123456',
        contactName: 'Demo Contractor Representative',
        email: 'contractor@easterncoking.com',
        phone: '+91 9876543210',
    ...
    const worker1 = await prisma.contractorWorker.upsert({
      ...
      create: {
        contractorId: contractor1.id,
        employeeCode: 'DEC-WRK-001',
        fullName: 'Demo Excavator Operator A',
        phone: '+91 9123456789',
    ...
    const worker2 = await prisma.contractorWorker.upsert({
      ...
      create: {
        contractorId: contractor1.id,
        employeeCode: 'DEC-WRK-002',
        fullName: 'Demo Dumper Operator B',
        phone: '+91 9832109876',
