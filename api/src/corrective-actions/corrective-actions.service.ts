...
export class CorrectiveActionsService {
  ...
  async create(
    ...
    const title = dto.title || `CAPA: ${dto.description.substring(0, 50)}`;
    const targetDue = dto.dueAt || dto.targetDate || new Date(Date.now() + 7 * 86400000).toISOString();
    const assigneeId = dto.assignedToId || user.id;
    ...
    if (!assignee || assignee.status !== UserStatus.ACTIVE) {
      throw new BadRequestException({
        ...
        message: `Assignee user "${assigneeId}" not found or is not ACTIVE`,
      });
    }
