  ...
  async createMany(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const item of items) {
        ...
        if (item.category && !Object.values(ComplianceCategory).includes(item.category as any)) {
          const str = String(item.category).toUpperCase();
          if (str.includes('SAFETY')) normalizedCategory = ComplianceCategory.SAFETY;
          else if (str.includes('ENV')) normalizedCategory = ComplianceCategory.ENVIRONMENT;
          else if (str.includes('LAB') || str.includes('WAGE') || str.includes('WORK')) normalizedCategory = ComplianceCategory.LABOUR;
          else if (str.includes('PROD')) normalizedCategory = ComplianceCategory.PRODUCTION;
          else normalizedCategory = undefined;
        }

        let normalizedFindingType: FindingType = FindingType.NOTE;
        if (item.findingType) {
          const fStr = String(item.findingType).toUpperCase();
          if (fStr.includes('UNSAFE')) normalizedFindingType = FindingType.UNSAFE_CONDITION;
          else if (fStr.includes('NON') || fStr.includes('COMPLIAN')) normalizedFindingType = FindingType.NON_COMPLIANCE;
          else normalizedFindingType = FindingType.NOTE;
        }

        let normalizedSeverity: ObservationSeverity = ObservationSeverity.MEDIUM;
        if (item.severity) {
          const sStr = String(item.severity).toUpperCase();
          if (sStr === 'LOW') normalizedSeverity = ObservationSeverity.LOW;
          else if (sStr === 'HIGH') normalizedSeverity = ObservationSeverity.HIGH;
          else if (sStr === 'CRITICAL') normalizedSeverity = ObservationSeverity.CRITICAL;
          else normalizedSeverity = ObservationSeverity.MEDIUM;
        }
        ...
        const obs = await tx.observation.create({
          data: {
            ...
            findingType: normalizedFindingType,
