...
export class ReportsController {
  ...
  @Get('risk-dossier/:mineId')
  async downloadRiskDossierByParam(
    @Param('mineId') mineId: string,
    @CurrentUser() user: RequestUser,
    @Res() res: Response,
  ) {
    const result = await this.reportsService.generateRiskDossierPdf(mineId, user);
  ...
  @Get('risk-dossier')
  async downloadRiskDossierByQuery(
