...
export class InspectionsService {
  ...
  /**
   * Start an inspection (SCHEDULED -> IN_PROGRESS) with GPS validation.
   */
  async start(
    id: string,
    user: RequestUser,
    dto?: { latitude?: number; longitude?: number; accuracy?: number; timestamp?: string },
  ): Promise<any> {
    const inspection = await this.prisma.inspection.findUnique({
      where: { id },
      include: { mine: true },
    ...
    let locationStatus = 'NO_GPS_DATA';
    let distanceKm: number | null = null;

    if (dto?.latitude !== undefined && dto?.longitude !== undefined) {
      locationStatus = 'VERIFIED_ONSITE';
      // If mine has a geoBoundary or center, check distance
      const geo: any = inspection.mine.geoBoundary;
      if (geo) {
        if (geo.lat && geo.lng) {
          const R = 6371;
          const dLat = ((geo.lat - dto.latitude) * Math.PI) / 180;
          const dLon = ((geo.lng - dto.longitude) * Math.PI) / 180;
          const a =
          ...
          const radius = geo.radius_km || 5.0;
          locationStatus = distanceKm <= radius ? 'VERIFIED_ONSITE' : 'OUTSIDE_GEOFENCE';
        }
      }
    }
    ...
    return {
      ...
      location: {
        ...
        capturedAt: dto?.timestamp || new Date().toISOString(),
        distanceKm,
      },
    };
  }
