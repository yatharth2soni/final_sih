...
export class AuditService {
  ...
  async verifyChain(dto: VerifyAuditChainDto, user: RequestUser) {
    // Authorized users may run chain verifications
    if (
      user.role !== UserRole.ADMIN &&
      user.role !== UserRole.REGULATOR &&
      user.role !== UserRole.CORPORATE_MANAGEMENT &&
  ...
  async getLogs(query: QueryAuditLogsDto, user: RequestUser): Promise<PaginatedResponse<any>> {
    ...
    if (user.role === UserRole.CORPORATE_MANAGEMENT || user.role === UserRole.CONTRACTOR) {
      const dbUser = await this.prisma.user.findUnique({
        where: { id: user.id },
        select: { companyId: true },
      });
      where.companyId = dbUser?.companyId || undefined;
