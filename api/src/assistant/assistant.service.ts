...
export class AssistantService {
  ...
  async processQuery(
    ...
    // Extract mine entity from natural language question
    const { mine: extractedMine, isExplicitlyMentioned } = await this.extractMineFromQuestion(question);

    let targetMineId: string | undefined;

    if (isExplicitlyMentioned && extractedMine) {
      // User explicitly named a mine in the query text -> assert user access to that exact mine
      await this.scopeService.assertMineAccess(user, extractedMine.id);
      targetMineId = extractedMine.id;
    } else if (dto.mineId) {
      await this.scopeService.assertMineAccess(user, dto.mineId);
      targetMineId = dto.mineId;
    ...
    // 3. Security Scope & Injection Verification
    if (!this.aiOrchestrator['scopeGuard'].isWithinScope(question)) {
      const rej = this.aiOrchestrator['scopeGuard'].getScopeRejectionResponse(lang);
      return {
        answer: rej.answer,
        language: lang,
        ...
        citations: [],
        limitations: ['Prompt rejected by ScopeGuard security policy.'],
        disclaimer: rej.disclaimer,
        provider: rej.provider,
      };
    }

    // 4. Dispatch to server-owned domain handlers based on allowlisted intent
    switch (intent) {
      case AssistantIntent.MINE_RISK:
        if (isMultiMineRisk && !isExplicitlyMentioned) {
          return this.handleMultiMineRisk(accessibleMineIds, lang, user);
        }
  ...
  /**
   * Identifies if a specific mine is named in the user query string in English or Hindi.
   */
  private async extractMineFromQuestion(
    question: string,
  ): Promise<{ mine: any | null; isExplicitlyMentioned: boolean }> {
    const qLower = question.toLowerCase();

    const allMines = await this.prisma.mine.findMany({
      where: { status: 'ACTIVE' },
      select: { id: true, name: true, code: true, location: true, companyId: true },
    });

    const aliases: Record<string, string[]> = {
      'jha': ['jharia', 'झरिया', 'dhanbad', 'धनबाद'],
      'krb': ['korba', 'कोरबा', 'chhattisgarh'],
      'rng': ['raniganj', 'रानीगंज', 'asansol', 'आसनसोल'],
      'sng': ['singrauli', 'सिंगरौली'],
      'uda': ['udaipur', 'उदयपुर', 'rajasthan'],
    };

    const stopWords = new Set([
      'mine', 'mines', 'the', 'for', 'and', 'what', 'risk', 'score',
      'status', 'open', 'show', 'list', 'block', 'pit', 'seam',
      'coal', 'safety', 'level', 'assistant', 'खदान', 'जोखिम', 'स्कोर'
    ]);

    const candidateMatches: Array<{ phrase: string; mine: any; priority: number }> = [];

    for (const mine of allMines) {
      // 1. Full mine name (Highest priority)
      candidateMatches.push({ phrase: mine.name.toLowerCase(), mine, priority: 100 });
      // 2. Full mine code
      candidateMatches.push({ phrase: mine.code.toLowerCase(), mine, priority: 90 });

      // 3. Regional / Hindi Aliases
      for (const [key, aliasList] of Object.entries(aliases)) {
        if (mine.code.toLowerCase().includes(key) || mine.name.toLowerCase().includes(key)) {
          aliasList.forEach((alias) => {
            candidateMatches.push({ phrase: alias.toLowerCase(), mine, priority: 80 });
          });
        }
      }

      // 4. Distinctive tokens in mine name
      const tokens = mine.name.toLowerCase().split(/[\s-_]+/);
      for (const tok of tokens) {
        if (tok.length >= 3 && !stopWords.has(tok)) {
          candidateMatches.push({ phrase: tok, mine, priority: 50 });
        }
      }
    }

    // Sort candidates: longest phrase first, then higher priority
    ...
    for (const candidate of candidateMatches) {
      if (qLower.includes(candidate.phrase)) {
        return { mine: candidate.mine, isExplicitlyMentioned: true };
      }
    }

    return { mine: null, isExplicitlyMentioned: false };
  }

  private async handleMultiMineRisk(
    ...
    } else if (user.role === UserRole.CORPORATE_MANAGEMENT) {
    ...
    const results = await Promise.all(
      mines.map(async (mine) => {
        const [riskScore, anomaliesCount] = await Promise.all([
          this.prisma.riskScore.findFirst({
            where: { mineId: mine.id },
            orderBy: { calculatedAt: 'desc' },
          }),
          this.prisma.anomalyFlag.count({
            where: { mineId: mine.id, status: 'OPEN' },
          }),
        ]);
        return { mine, riskScore, anomaliesCount };
      }),
    );

    return ResponseGenerator.formatMultiMineRisk(results, lang);
  }
