...
export class ResponseGenerator {
  ...
  /**
   * Generates bilingual structured response for multi-mine risk queries (e.g. "Which mines are high risk?").
   */
  static formatMultiMineRisk(minesList: Array<{ mine: any; riskScore: any; anomaliesCount: number }>, lang: 'en' | 'hi'): FormattedResponse {
    const isHi = lang === 'hi';
    const citations: AssistantCitation[] = [];

    if (!minesList || minesList.length === 0) {
      const answer = isHi
        ? 'आपके अधिकार क्षेत्र में कोई सक्रिय खदान नहीं पाई गई।'
        : 'No active mines found within your authorized scope.';
      return {
        answer,
        language: lang,
        intent: AssistantIntent.MINE_RISK,
    ...
    let answer = '';
    if (isHi) {
      answer = `**अधिकार क्षेत्र में खदान सुरक्षा जोखिम सारांश (${minesList.length} खदानें):**\n` +
        minesList
          .map((item, i) => {
            const mName = item.mine.name;
            const score = item.riskScore ? item.riskScore.score : 0;
            const band = item.riskScore ? (item.riskScore.band || 'LOW') : 'LOW';
            const anom = item.anomaliesCount > 0 ? ` [⚠️ ${item.anomaliesCount} असामान्यता ध्वज]` : '';
            return `${i + 1}. **${mName}** — जोखिम स्कोर: **${score} / 100** (${band})${anom}`;
          })
          .join('\n');
    } else {
      answer = `**Mine Safety Risk Summary across your authorized scope (${minesList.length} mines):**\n` +
        minesList
          .map((item, i) => {
            const mName = item.mine.name;
            const score = item.riskScore ? item.riskScore.score : 0;
            const band = item.riskScore ? (item.riskScore.band || 'LOW') : 'LOW';
            const anom = item.anomaliesCount > 0 ? ` [⚠️ ${item.anomaliesCount} active anomaly flag(s)]` : '';
            return `${i + 1}. **${mName}** — Risk Score: **${score} / 100** (${band})${anom}`;
          })
          .join('\n');
    }
    ...
    minesList.forEach(item => {
      citations.push({
        resourceType: 'Mine',
        resourceId: item.mine.id,
        label: `${item.mine.name} (${item.mine.code}): Score ${item.riskScore?.score || 0} (${item.riskScore?.band || 'LOW'})`,
      });
    });
    ...
    return {
      ...
      limitations: [
        isHi
          ? 'जोखिम स्कोर पिछले 30/90 दिनों के उल्लंघनों, खुले कापा और शिकायतों के भारित योग पर आधारित है।'
          : 'Risk scores are calculated deterministically based on rolling 30/90 day weighted violations, open CAPAs, and grievances.',
      ],
      disclaimer: isHi ? STATUTORY_DISCLAIMER_HI : STATUTORY_DISCLAIMER_EN,
      provider: 'deterministic',
    };
