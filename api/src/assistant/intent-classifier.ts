...
const INTENT_RULES: Array<{
...
}> = [
  {
    ...
    patterns: [
      /\b(help|capabilities|what\s+can\s+you\s+do|commands|hi|hello|hey)\b/i,
      /(मदद|सहायता|क्षमता|क्या\s+कर\s+सकते|नमस्ते|नमस्कार|हेलो)/,
    ],
  },
  {
...
export function classifyIntent(
  ...
  for (const rule of INTENT_RULES) {
    ...
    for (const keyword of rule.keywords) {
      ...
      // For short words (e.g. 'hi'), enforce word boundary to prevent matching inside 'which' or 'high'
      if (kw.length <= 4 && !/[\u0900-\u097F]/.test(kw)) {
